...
$('#titulos_data').DataTable({
    ...
    "ajax":{
        url:"../../controller/titulo.php?op=listar_titulos_pendiente_alcalde",
        type:"post"
    },
    "bDestroy": true,
    "responsive": false,
...
function firmar(titu_id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.post("../../controller/titulo.php?op=cambiar_estfirmado4", { titu_id: titu_id })
                .done(function() {
                    // Segunda llamada AJAX para cambiar el estado a no impreso
                    return $.post("../../controller/titulo.php?op=cambiar_estcompleto", { titu_id: titu_id });
                })
                .done(function() {
                    // Segunda llamada AJAX para cambiar el estado a no impreso
...
function imp_masivo(){
    ...
    redirect_by_post(
        "../../controller/titulo.php?op=imprimir_x_firm3"
...
function total_titulos_pendientes(){
    $.post("../../controller/titulo.php?op=total_pendientes_alcalde", function (data) {
...
function total_titulos_descargables(){
    $.post("../../controller/titulo.php?op=total_descargables_alcalde", function (data) {
